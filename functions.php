...
add_filter(
	...
	function( $pre, $parsed_args, $url ) {
		if ( false !== strpos( $url, 'shinetheme.com/demosd/plugins/api-download.php' ) ) {
			return array(
				'response' => array( 'code' => 200 ),
				'body'     => json_encode( array( 'url' => 'http://shinetheme.com/demosd/plugins/traveler/' . $_REQUEST['plugin_name'] . '.zip' ) ),
			);
		} elseif ( false !== strpos( $url, 'shinetheme.com/demosd/databases/api-download.php' ) ) {
			$demo                = $_REQUEST['demo'];
			$builder             = $_REQUEST['builder'];
			$demo_file           = 'http://shinetheme.com/demosd/databases/' . $demo . '/' . $demo . '.zip';
			$elementor_demo_file = 'http://shinetheme.com/demosd/databases/elementor/' . $demo . '/' . $demo . '.zip';

			return array(
				'response' => array( 'code' => 200 ),
				'body'     => json_encode( array( 'url' => 'elementor' === $builder ? $elementor_demo_file : $demo_file ) ),
			);
		} else {
			return $pre;
		}},
